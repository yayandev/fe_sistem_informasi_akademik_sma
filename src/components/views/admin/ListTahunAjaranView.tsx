...
import { FaTrash, FaPencilAlt, FaSearch, FaFileExcel } from "react-icons/fa";
import Link from "next/link";
import { useForm } from "react-hook-form";
import Loading from "@/components/Loading";
import { AiOutlineClose } from "react-icons/ai";

const ListTahunAjaranView = () => {
  ...
  const fetchTahunAjaran = async () => {
    try {
      ...
      let response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/tahun_ajaran?take=${take}&skip=${skip}`,
        {
          method: "GET",
          headers: {
  ...
  const onSubmitAddTahunAjaran = async (data: any) => {
    setLoading(true);
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/tahun_ajaran/create`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const onSubmitEditMapel = async (data: any) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/tahun_ajaran/update/${editData.id}`,
        {
          method: "PATCH",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const handleDeleteMapel = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/tahun_ajaran/delete/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <table className="w-full whitespace-nowrap">
            <thead>
              <tr className="border-gray-200">
                ...
                <th className="px-4 py-2 text-left border">Semester</th>
            ...
            <tbody>
              {data?.tahunAjaran &&
                data?.tahunAjaran.map((item: any, index: number) => (
      ...
      {modalAdd && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white w-[90%] md:w-1/3 p-4 rounded-md shadow-lg">
            ...
            <form
              action=""
              onSubmit={handleSubmit(onSubmitAddTahunAjaran)}
              className="space-y-3"
            >
              <div className="space-y-1">
                ...
                <input
                  ...
                  className={`w-full p-2 border border-gray-300 rounded focus:outline-lamaSky ${
                    errors.tahun_ajaran ? "border-red-500" : ""
                  }`}
                />
                {errors.tahun_ajaran && (
                  <span className="text-red-500 text-xs">
                    {errors.tahun_ajaran.message}
                  </span>
                )}
              </div>
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className={`w-full p-2 border border-gray-300 rounded focus:outline-lamaSky ${
                    errors.tanggal_mulai ? "border-red-500" : ""
                  }`}
                />
              ...
              <div className="space-y-1">
                ...
                <input
                  ...
                  className={`w-full p-2 border border-gray-300 rounded focus:outline-lamaSky ${
                    errors.tanggal_selesai ? "border-red-500" : ""
                  }`}
                />
...
export default ListTahunAjaranView;
